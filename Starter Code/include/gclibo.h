...
#define POLLINGINTERVAL 100 //interval for polling commands, in miliseconds.
	...
	*  \return The success status or error code of the function. See gclib_errors.h for possible values.
	*
	*  Each line of the returned data will be of the form *address, revision report* (if available).
	*  If an IP address is unreachable via ping, the address will be in parentheses.
	*  \code{.unparsed}
	*  192.168.0.42, DMC4080 Rev 1.2d
	*  192.168.0.80, RIO47122 Rev 1.1h
	*  (10.4.0.12), DMC30010 Rev 1.2e
	*  COM1
	*  \endcode
	*/
	...
	*
	*  \note Linux/OS X users must be root to use GIpRequests() and have UDP access to bind and listen on port 67.
	*
	*  \code{.cpp}
	...
	*
	*  \note Linux/OS X users must be root to use GAssign() and have UDP access to send on port 68.
	*
	*  \code{.cpp}
